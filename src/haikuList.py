...
def insert(first:str,second:str,third:str,poster:str,detail:str):
    try:
        ...
        conn.commit()
        conn.close()
        result = const.HaikuListStatusResult(True,"")
    except Exception as e:
        #エラー出たらエラー内容と追加失敗を返す
        result = const.HaikuListStatusResult(False,str(e))
    return result
...
SELECT
"""
def select(id:int,haikuText:str,poster:str,detail:str) -> list[const.HaikuListRecord]:
    # データベースに接続する
    conn = sqlite3.connect(dbname)
    cursor = conn.cursor()
